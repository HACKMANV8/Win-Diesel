export function findDevtoMarkdownTextarea(): HTMLTextAreaElement | null {
  ...
  const ta = document.querySelector('textarea[name="body_markdown"]') as HTMLTextAreaElement | null;
  if (ta) return ta;

  // Fallbacks observed in some layouts/tests
  const byId = document.getElementById('article-body') as HTMLTextAreaElement | null;
  if (byId && byId.tagName === 'TEXTAREA') return byId;

  // Last-resort: any visible textarea within main article form
  const candidates = Array.from(document.querySelectorAll('form textarea')) as HTMLTextAreaElement[];
...
export function appendToDevtoEditor(text: string): boolean {
  ...
  const editable = document.querySelector('[contenteditable="true"], div[role="textbox"]') as HTMLElement | null;
  if (editable) {
...
export function replaceInDevtoEditor(searchText: string, replaceWith: string): boolean {
  const textarea = findDevtoMarkdownTextarea();
  ...
  if (editable) {
    ...
    while (current) {
      ...
      if (idx >= 0) {
        ...
        tn.textContent = txt.slice(0, idx) + replaceWith + txt.slice(idx + searchText.length);
        return true;
      }
...
function isInsideMarkdownLink(content: string, start: number, end: number): boolean {
